...
**Mini Window Application Management**

In multi-window mode, the SDK supports inserting PPT or whiteboard plugins as mini window applications. Starting from this version, the `WhiteRoom` class introduces the following methods to manage mini window applications in multi-window mode:

- `addApp`: Inserts a mini window application and returns the application ID.
- `closeApp`: Closes the window of the specified mini window application.
- `focusApp`: Switches the focused window to the window of the specified mini window application.
- `queryApp`: Queries the information of the specified mini window application.
- `queryAllAppsWithCompletionHandler`: Queries the information of all current mini window applications.

**Customization of Whiteboard Tools**
