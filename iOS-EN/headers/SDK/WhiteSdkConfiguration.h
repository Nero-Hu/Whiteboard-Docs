...
/**
 ...
 - `YES`：Enable server-side typesetting.
 - `NO`：(Default) Disable server-side typesetting.
...
/**
 ...
 - `YES`：Preload all image resources in dynamic PPT slides when loading the homepage of the slides.
 - `NO`：Do not preload all image resources in dynamic PPT slides when loading the homepage of the slides.
 */
...
/**
 ...
 For example, if you set the log level to `info`, the SDK outputs the logs within levels `error`，`warn`，and `info`.
  ...
  - Log information reporting mode: Optional. The default value is `info`.
  - Local client: user connection quality reporting mode. The default value is `Report`.
  - Log reporting level: Optional. The default value is `info`.
  - Webview console print log level: Optional. The default value is `info`.
  - Turn off the log upload function: Enabled by default.
 */
