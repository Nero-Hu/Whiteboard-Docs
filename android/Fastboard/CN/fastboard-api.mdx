...
<H3 className="anchor index-api-registerapp" id="registerapp">registerApp</H3>
...
- `params`：注册参数，包含应用配置。详见 <a href="#fastregisterappparams">FastRegisterAppParams</a>。
- `result`：`registerApp` 方法调用的结果。详见 <a href="#result">FastResult</a>。传入 `FastResult` 实例，SDK 会触发在 `FastResult` 接口中实现的回调，报告 `registerApp` 方法调用是否成功；传入 `null` 表示不监听回调。


<a name="fastregisterappparams"></a>
<H3 className="anchor index-api-fastregisterappparams" id="fastregisterappparams">FastRegisterAppParams</H3>
...
<H3 className="anchor index-api-setstrokerange" id="setstrokerange">setStrokeRange</H3>
...
该方法配置工具箱扩展面板中线条宽度滑块的最小值和最大值。用户在使用铅笔、矩形、圆形等绘图工具时，可以在此范围内调整线条宽度。

**默认范围**

1 到 24（在 ExtensionLayout 中定义）
