...
- `enableAppInMainViewPlugin`: （可选）启用 [`@netless/app-in-mainview-plugin`](https://www.npmjs.com/package/@netless/app-in-mainview-plugin) 插件，支持在白板主视图管理[小窗应用](/doc/whiteboard/javascript/fastboard-sdk/overview/concepts#小窗应用)：
  - `true`：采用默认配置启用插件。
