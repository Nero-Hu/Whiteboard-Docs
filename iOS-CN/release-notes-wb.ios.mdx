...
<VersionSection version="2.16.61 版">
...
`WhiteSdkConfiguration` 新增 `enableSlideInterrupterAPI` 配置项。开启后，通过调用 `WhiteSDK.setSlideDelegate` 设置回调对象实现 PPT URL 的拦截替换功能。

<ListTitle>PPT 显示配置</ListTitle>
...
如果你将 SDK 升级到 v2.15.0 或之后版本，要加入房间，你必须调用 `joinRoomWithConfig`，并确保初始化 `WhiteRoomConfig` 对象时在 `initWithUuid` 中传入字符串格式的 `uid`。每个用户的 `uid` 长度不能超过 1024 字节，且在同一房间内必须唯一。详见《[加入实时房间](https://docs.agora.io/cn/whiteboard/join_whiteboard_room_ios?platform=iOS#3-初始化-sdk-并加入房间)》。
